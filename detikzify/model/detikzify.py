    ...
    def forward(
        ...
        if self.get_vision_tower() and (input_ids.shape[1] != 1 or self.training) and images is not None:
            ...
            for cur_input_ids, cur_input_embeds in zip(input_ids, inputs_embeds):
                ...
                found_patch_tokens = (cur_input_ids == self.config.patch_token_id).sum().item()
                masked_indices = torch.where(cur_input_ids == self.config.patch_token_id)[0]
                if found_patch_tokens != num_patches:
                    # Adaptive recovery: adjust to min(found, expected) to avoid crash
                    adapt_n = min(found_patch_tokens, num_patches)
                    # Ensure masked_indices sorted
                    masked_indices_sorted = torch.sort(masked_indices).values
                    # Try to find a consecutive window of length adapt_n starting from first index
                    if masked_indices_sorted.numel() >= adapt_n:
                        # Identify first window
                        base_start = masked_indices_sorted[0]
                        window = torch.arange(base_start, base_start + adapt_n, device=masked_indices_sorted.device)
                        # If window not fully present, just take first adapt_n indices as fallback
                        if (masked_indices_sorted[:adapt_n] != window).any():
                            window = masked_indices_sorted[:adapt_n]
                    else:
                        window = masked_indices_sorted
                        adapt_n = window.numel()
                    if not hasattr(self, '_patch_mismatch_warned'):
                        try:
                            print(f"[DetikzifyModel] ?? Patch token mismatch recovered: expected={num_patches} found={found_patch_tokens} using={adapt_n}")
                        except Exception:
                            pass
                        self._patch_mismatch_warned = 1
                    # Adjust image features length as needed
                    adj_image_feats = cur_image_features[:adapt_n]
                    mask_index_start = window[0]
                    cur_new_input_embeds = torch.cat((cur_input_embeds[:mask_index_start], adj_image_feats, cur_input_embeds[mask_index_start+adapt_n:]), dim=0)
                else:
                    mask_index_start = masked_indices[0]
                    if (masked_indices != torch.arange(mask_index_start, mask_index_start+num_patches, device=masked_indices.device, dtype=masked_indices.dtype)).any():
                        if not hasattr(self, '_patch_non_consecutive_warned'):
                            try:
                                print("[DetikzifyModel] ?? Non-consecutive patch tokens detected; attempting best-effort insertion")
                            except Exception:
                                pass
                            self._patch_non_consecutive_warned = 1
                        # Best-effort: treat first contiguous stretch
                        contiguous_len = 1
                        for idx_j in range(1, masked_indices.numel()):
                            if masked_indices[idx_j] == masked_indices[idx_j-1] + 1:
                                contiguous_len += 1
                            else:
                                break
                        use_len = min(contiguous_len, num_patches)
                        cur_image_features_adj = cur_image_features[:use_len]
                        start_idx = masked_indices[0]
                        cur_new_input_embeds = torch.cat((cur_input_embeds[:start_idx], cur_image_features_adj, cur_input_embeds[start_idx+use_len:]), dim=0)
                    else:
